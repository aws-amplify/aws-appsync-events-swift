...
class TaskQueue<Success> {
    ...
    func async(block: @Sendable @escaping () async throws -> Success) {
        streamContinuation.yield {
            do {
                _ = try await block()
            } catch {
                print("Failed to handle async task in TaskQueue<\(Success.self)> with error: \(error)")
            }
        }
    }
}
